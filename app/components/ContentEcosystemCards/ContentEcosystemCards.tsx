"use client";

import React, { useRef, useEffect } from "react";
import Image from "next/image";
import styles from "./ContentEcosystemCards.module.css";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

const GreenCheck = () => (
  <svg className={styles.featureIcon} viewBox="0 0 24 24" fill="none">
    <circle cx="12" cy="12" r="12" fill="#22c55e" />
    <path
      d="M7 12.5L10 15.5L17 8.5"
      stroke="white"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

const cardsData = [
  {
    id: "content-creation",
    label: "BRAND STORYTELLING",
    title: "Content",
    titleHighlight: "Creation",
    description:
      "Premium visual storytelling designed to capture audience attention and elevate the brand experience.",
    features: [
      {
        title: "Brand Storytelling",
        description:
          "Creative storytelling to showcase brand identity, positioning, and core values.",
        icon: <GreenCheck />,
      },
      {
        title: "Conversion-Focused Design",
        description:
          "Ad creatives built specifically to boost clicks and drive purchases.",
        icon: <GreenCheck />,
      },
      {
        title: "Platform-Ready Assets",
        description:
          "Videos and graphics optimized to perform best on native social feeds.",
        icon: <GreenCheck />,
      },
    ],
    imageSrc: "/content-creation.webp", // Replace with actual path later
    imageBg: "#E8EAF6",
  },
  {
    id: "content-distribution",
    label: "PLATFORM BRAND EXPERIENCE",
    title: "Content",
    titleHighlight: "Distribution",
    description:
      "Creating immersive brand storefronts that enhance product discovery and drive higher conversion.",
    features: [
      {
        title: "Platform Adaptation",
        description:
          "Customising content formats for marketplaces, social media and ads.",
        icon: <GreenCheck />,
      },
      {
        title: "Multi-Channel Publishing",
        description:
          "Ensuring consistent brand presence across digital commerce ecosystems.",
        icon: <GreenCheck />,
      },
      {
        title: "Creator Collaboration",
        description:
          "Extended reach and audience targeting through influencer partnerships.",
        icon: <GreenCheck />,
      },
    ],
    imageSrc: "/content-distribution.webp", // Replace with actual path
    imageBg: "#E3F2FD",
  },
  {
    id: "analytics-reporting",
    label: "PLATFORM BRAND EXPERIENCE",
    title: "Analytics &",
    titleHighlight: "Reporting",
    description:
      "Data-driven insights that measure performance and guide strategic decisions.",
    features: [
      {
        title: "Performance Tracking",
        description:
          "Monitoring key metrics across campaigns, content and platforms.",
        icon: <GreenCheck />,
      },
      {
        title: "Consumer Insights",
        description:
          "Understanding behaviour patterns to refine targeting and messaging.",
        icon: <GreenCheck />,
      },
      {
        title: "Growth Optimisation",
        description:
          "Using insights to continuously improve marketing efficiency and outcomes.",
        icon: <GreenCheck />,
      },
    ],
    imageSrc: "/analytics-reporting.webp", // Replace with actual path
    imageBg: "#E8F5E9",
  },
];

export default function ContentEcosystemCards() {
  const containerRef = useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    // Optional GSAP animation to slightly scale down previous cards as next one slides over
    const ctx = gsap.context(() => {
      const cards = gsap.utils.toArray<HTMLElement>(`.${styles.card}`);
      
      cards.forEach((card, index) => {
        if (index === cards.length - 1) return; // Last card doesn't need to scale down
        
        gsap.to(card, {
          scale: 0.95, // Scale down slightly as user scrolls past it
          opacity: 0.8,
          ease: "none",
          scrollTrigger: {
            trigger: card,
            start: "top 80px", // When card hits sticky top
            endTrigger: cards[index + 1], // Until the next card reaches top
            end: "top 80px",
            scrub: true,
          }
        });
      });
    }, containerRef);
    
    return () => ctx.revert();
  }, []);

  return (
    <section className={styles.cardsSection}>
      <div className={styles.container} ref={containerRef}>
        <div className={styles.cardsWrapper}>
          {cardsData.map((card, index) => (
            <div
              key={card.id}
              className={`${styles.card} ${index % 2 !== 0 ? styles.cardReverse : ""}`}
              style={{
                top: `calc(80px + ${index * 20}px)`,
                zIndex: index,
              }}
            >
              {/* Left Side: Content */}
              <div className={styles.cardLeft}>
                <div className={styles.labelWrapper}>
                  <div className={styles.labelLine}></div>
                  <div className={styles.labelText}>{card.label}</div>
                </div>
                
                <h2 className={styles.title}>
                  {card.title} <span className={styles.highlightBlue}>{card.titleHighlight}</span>
                </h2>
                
                <p className={styles.description}>{card.description}</p>
                
                <div className={styles.featuresList}>
                  {card.features.map((feature, i) => (
                    <div key={i} className={styles.featureItem}>
                      {feature.icon}
                      <div className={styles.featureText}>
                        <h4 className={styles.featureTitle}>{feature.title}</h4>
                        <p className={styles.featureDesc}>{feature.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
              </div>

              {/* Right Side: Image/Box Content */}
              <div className={styles.cardRight} style={{ backgroundColor: card.imageBg }}>
                {card.imageSrc && (
                  <Image
                    src={card.imageSrc}
                    alt={card.title}
                    fill
                    className={styles.cardImage}
                    sizes="(max-width: 900px) 100vw, 50vw"
                    priority={index === 0} // prioritize loading the first card image
                  />
                )}
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
